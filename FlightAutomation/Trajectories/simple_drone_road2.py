# Create a smoothed-turn version of the user's simple_drone_road.py, keeping the same topics/QoS and overall style.
# We add Chaikin smoothing + densification, raise lookahead, and leave altitude constant from odom.
# Save to /mnt/data/simple_drone_road_smooth.py
...
def densify_polyline(points: List[Tuple[float, float]], spacing: float = 1.0) -> List[Tuple[float, float]]:
    ...
    for i in range(len(points) - 1):
        ...
        while dist + spacing <= seg_len:
            dist += spacing
            out.append((x0 + nx * dist, y0 + ny * dist))
...
class SimpleDroneController(Node):
    def __init__(self):
        ...
        self.velocity = 8.0             # 水平速度 (m/s)
        self.lookahead = 8.0            # 更大的前瞻：更平滑的转向
        self.adaptive_lookahead = 0.0   # 如需自适应可设 >0
        ...
        # —— 目标路径（保持高度不变；此处示例与原脚本一致，可自行替换/扩展） —— #
        ...
        # —— 平滑与采样参数 —— #
        self.smooth_iters = 3          # Chaikin 迭代次数：越大越圆滑
        self.chaikin_weight = 0.25     # 角切比例（经典 0.25/0.75）
        self.target_spacing = 1.0      # 加密间距（米）：越小越细腻
    ...
    def _publish_once(self):
        ...
        # 1) 平滑路径（不过锚点）
        smoothed = chaikin_smooth(self.xy_points, iters=self.smooth_iters, weight=self.chaikin_weight)
